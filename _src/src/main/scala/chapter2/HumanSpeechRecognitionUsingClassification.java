...
/**
 * Given a movie review (raw text),
 * classify that movie review as either positive or negative
 * based on the words it contains.
 */
public class HumanSpeechRecognitionUsingClassification {
    ...
    private static final String IMDB_COMMENTS_URL =
        "http://ai.stanford.edu/~amaas/data/sentiment/aclImdb_v1.tar.gz";
    private static final String IMDB_DATA_PATH =
    ...
    public static void main(String[] args) throws Exception {
        ...
        WordVectors wordVectors =
            WordVectorSerializer
                .loadStaticModel(new File(GOOGLE_NEWS_VECTOR_PATH));
        DataSetIteratorWord2Vec train =
            new DataSetIteratorWord2Vec(IMDB_DATA_PATH,
                wordVectors, BATCH_SIZE,
                MAX_NUMBER_OF_WORDS_TAKEN_FROM_REVIEW, true);
        DataSetIteratorWord2Vec test =
            new DataSetIteratorWord2Vec(IMDB_DATA_PATH, wordVectors,
                BATCH_SIZE, MAX_NUMBER_OF_WORDS_TAKEN_FROM_REVIEW, false);

        performTraining(net, train, test);
        printFirstPredictedPositiveReview(net, test);
    }

    private static void printFirstPredictedPositiveReview(MultiLayerNetwork net, DataSetIteratorWord2Vec test) throws IOException {
        //After training: load a single example and generate predictions
        File firstPositiveReviewFile = new File(
            FilenameUtils.concat(IMDB_DATA_PATH,
                "aclImdb/test/pos/0_10.txt"));
        String firstPositiveReview = FileUtils.readFileToString(firstPositiveReviewFile);

        INDArray features = test
            .transformStringIntoFeatureVectorOfNumber(firstPositiveReview, MAX_NUMBER_OF_WORDS_TAKEN_FROM_REVIEW);
        INDArray networkOutput = net.output(features);
        long timeSeriesLength = networkOutput.size(2);
        INDArray probabilitiesAtLastWord = networkOutput
            .get(NDArrayIndex.point(0), NDArrayIndex.all(), NDArrayIndex.point(timeSeriesLength - 1));

        System.out.println("\n\n-------------------------------");
        System.out.println("First positive review: \n" + firstPositiveReview);
        System.out.println("\n\nProbabilities at last time step:");
        System.out.println("p(positive): " + probabilitiesAtLastWord
            .getDouble(0));
        System.out.println("p(negative): " + probabilitiesAtLastWord
            .getDouble(1));
    }

    private static void performTraining(MultiLayerNetwork net,
                                        DataSetIteratorWord2Vec train,
                                        DataSetIteratorWord2Vec test) {
