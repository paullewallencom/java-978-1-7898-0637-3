...
public class NaturalLanguageModelingUsingRNN {

	// define a sentence to learn.
    // Add a special character at the beginning so the RNN learns the complete string and ends with the marker.
	private static final char[] LEARN_STRING =
			"* he cleaning agent cleans the mailbox.".toCharArray();

	// a list of all possible characters
	private static final List<Character> LEARN_STRING_CHARS_LIST =
			new ArrayList<>();
	...
	public static void main(String[] args) {
		...
		LinkedHashSet<Character> LEARN_STRING_CHARS =
		...
		RnnOutputLayer.Builder outputLayerBuilder =
				normalizeOutputOfNeurons(LEARN_STRING_CHARS);
		...
		for (int epoch = 0; epoch < 500; epoch++) {
			...
			INDArray testInit = Nd4j.zeros(1,
					LEARN_STRING_CHARS_LIST.size(), 1);
			testInit.putScalar(LEARN_STRING_CHARS_LIST.indexOf(
					LEARN_STRING[0]), 1);
	...
	private static DataSet createTrainingData() {
		...
		// SEQUENCE_POSITION
		INDArray input = Nd4j
				.zeros(1, LEARN_STRING_CHARS_LIST.size(),
						LEARN_STRING.length);
		INDArray labels = Nd4j
				.zeros(1, LEARN_STRING_CHARS_LIST.size(), LEARN_STRING.length);
		int samplePos = 0;
	...
	private static void buildRNN(LinkedHashSet<Character> LEARN_STRING_CHARS, ListBuilder listBuilder) {
		...
		for (int i = 0; i < HIDDEN_LAYER_CONT; i++) {
			...
			hiddenLayerBuilder.nIn(i == 0 ? LEARN_STRING_CHARS.size() :
					HIDDEN_LAYER_WIDTH);
